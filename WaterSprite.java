...
/*Class Description
 * This class is a helper class in charge of importing the image for one of the elemental sprites (Water).
 * It also contains all methods to render the Water Sprite based on what is happening in the game.
*/
...
public class WaterSprite extends JComponent {

    public int positionX, positionY, dx, dy, width, height;
    private BufferedImage waterSpriteVisual;
    private Random random = new Random();
    private boolean imageLoaded = false;

    public WaterSprite(int x, int y, int dx, int dy) {
        positionX = x;
        positionY = y;
        this.dx = dx;
        this.dy = dy;
        loadObjImage();
        width = waterSpriteVisual.getWidth();
        height = waterSpriteVisual.getHeight();
    }

    private void loadObjImage() {
        if (!imageLoaded) {
            try {
                InputStream waterSpImports = getClass().getResourceAsStream("Sprite_Water_Visuals.png");
                waterSpriteVisual = ImageIO.read(waterSpImports);
                imageLoaded = true;
            } catch (IOException e) {
    ...
    public void renderWater(Graphics2D g2d) {
        g2d.drawImage(waterSpriteVisual, positionX, positionY, null);
    }
